...
enum Tab { #[default] LowPoly, Voronoi, Halftone, Topo, Mosaic, Engrave, Wfc, Crackle,
           FlowField, OpArt, Weave, StringArt, RD, Maze, Circuit, Bubble, Penrose, Output }

#[derive(PartialEq, Clone, Copy, Debug)]
enum Style { LowPoly, Voronoi, Halftone, Topo, Mosaic, Engrave, Wfc, Crackle,
             FlowField, OpArt, Weave, StringArt, RD, Maze, Circuit, Bubble, Penrose }
...
struct Params {
    ...
    sa_cx_a: f32, sa_cy_a: f32, sa_r_a: f32,
    sa_cx_b: f32, sa_cy_b: f32, sa_r_b: f32,
    sa_fg_color: [f32; 3],
    sa_bg_color: [f32; 3],
...
impl Default for Params {
    fn default() -> Self {
        Self {
            ...
            sa_n_circles:  1.0,
            sa_cx_a: 0.5, sa_cy_a: 0.5, sa_r_a: 0.42,
            sa_cx_b: 0.5, sa_cy_b: 0.5, sa_r_b: 0.28,
...
impl Params {
    ...
    fn to_ff_uniforms(&self) -> FlowFieldUniforms {
        let c = |v: [f32; 3]| Vec4::new(v[0], v[1], v[2], 1.0);
        FlowFieldUniforms {
            freq: self.ff_freq, n_lines: self.ff_n_lines,
            line_width: self.ff_line_width, warp: self.ff_warp,
            octaves: self.ff_octaves, color_var: self.ff_color_var,
            _pad0: 0.0, _pad1: 0.0,
            c_tl: c(self.ff_c_tl), c_tr: c(self.ff_c_tr),
            c_bl: c(self.ff_c_bl), c_br: c(self.ff_c_br),
        }
    }
    ...
    fn to_oa_uniforms(&self) -> OpArtUniforms {
        let c = |v: [f32; 3]| Vec4::new(v[0], v[1], v[2], 1.0);
        OpArtUniforms {
            ring_count: self.oa_ring_count, ring_width: self.oa_ring_width,
            warp: self.oa_warp, freq: self.oa_freq, twist: self.oa_twist,
            _pad0: 0.0, _pad1: 0.0, _pad2: 0.0,
            fg_color: c(self.oa_fg_color), bg_color: c(self.oa_bg_color),
        }
    }
    ...
    fn to_wv_uniforms(&self) -> WeaveUniforms {
        let c = |v: [f32; 3]| Vec4::new(v[0], v[1], v[2], 1.0);
        WeaveUniforms {
            thread_count: self.wv_thread_count, thread_width: self.wv_thread_width,
            shadow: self.wv_shadow, color_var: self.wv_color_var,
            _pad0: 0.0, _pad1: 0.0, _pad2: 0.0, _pad3: 0.0,
            warp_color: c(self.wv_warp_color), weft_color: c(self.wv_weft_color),
    ...
    fn to_sa_uniforms(&self) -> StringArtUniforms {
        let c = |v: [f32; 3]| Vec4::new(v[0], v[1], v[2], 1.0);
        StringArtUniforms {
            n_points: self.sa_n_points, k_offset: self.sa_k_offset,
            line_width: self.sa_line_width, n_circles: self.sa_n_circles,
            center_a: Vec4::new(self.sa_cx_a, self.sa_cy_a, self.sa_r_a, 0.0),
            center_b: Vec4::new(self.sa_cx_b, self.sa_cy_b, self.sa_r_b, 0.0),
            fg_color: c(self.sa_fg_color), bg_color: c(self.sa_bg_color),
        }
    }
    ...
    fn to_rd_uniforms(&self) -> RdUniforms {
        let c = |v: [f32; 3]| Vec4::new(v[0], v[1], v[2], 1.0);
        RdUniforms {
            scale: self.rd_scale, sharpness: self.rd_sharpness,
            warp: self.rd_warp, balance: self.rd_balance,
            _pad0: 0.0, _pad1: 0.0, _pad2: 0.0, _pad3: 0.0,
            fg_color: c(self.rd_fg_color), bg_color: c(self.rd_bg_color),
            c_tl: c(self.rd_c_tl), c_tr: c(self.rd_c_tr),
            c_bl: c(self.rd_c_bl), c_br: c(self.rd_c_br),
        }
    }
    ...
    fn to_mz_uniforms(&self) -> MazeUniforms {
        ...
        MazeUniforms {
            grid_w: self.mz_grid_w as u32, grid_h: self.mz_grid_h as u32,
            line_width: self.mz_line_width, _pad: 0.0,
            wall_color: c(self.mz_wall_color), floor_color: c(self.mz_floor_color),
            walls,
        }
    }
    ...
    fn to_cb_uniforms(&self) -> CircuitUniforms {
        let c = |v: [f32; 3]| Vec4::new(v[0], v[1], v[2], 1.0);
        CircuitUniforms {
            cell_size: self.cb_cell_size, trace_width: self.cb_trace_width,
            via_radius: self.cb_via_radius, density: self.cb_density,
            color_var: self.cb_color_var,
            _pad0: 0.0, _pad1: 0.0, _pad2: 0.0,
            trace_color: c(self.cb_trace_color), via_color: c(self.cb_via_color),
    ...
    fn to_bp_uniforms(&self) -> BubbleUniforms {
        let c = |v: [f32; 3]| Vec4::new(v[0], v[1], v[2], 1.0);
        BubbleUniforms {
            density: self.bp_density, min_r: self.bp_min_r, max_r: self.bp_max_r,
            border: self.bp_border, jitter: self.bp_jitter, color_var: self.bp_color_var,
            _pad0: 0.0, _pad1: 0.0,
            c_tl: c(self.bp_c_tl), c_tr: c(self.bp_c_tr),
            c_bl: c(self.bp_c_bl), c_br: c(self.bp_c_br),
        }
    }
...
impl Material2d for WfcMaterial {
    fn fragment_shader() -> ShaderRef { "shaders/wfc.wgsl".into() }
}

#[derive(Resource)] struct WfcMaterialHandle(Handle<WfcMaterial>);
#[derive(Resource)] struct WfcMeshHandle(Handle<Mesh>);
...
impl Material2d for CrackleMaterial {
    fn fragment_shader() -> ShaderRef { "shaders/crackle.wgsl".into() }
}

#[derive(Resource)] struct CrackleMaterialHandle(Handle<CrackleMaterial>);
#[derive(Resource)] struct CrackleMeshHandle(Handle<Mesh>);

// ── Flow Field material ────────────────────────────────────────────────────────

#[derive(Clone, ShaderType)]
struct FlowFieldUniforms {
    freq: f32, n_lines: f32, line_width: f32, warp: f32,
    octaves: f32, color_var: f32, _pad0: f32, _pad1: f32,
    c_tl: Vec4, c_tr: Vec4, c_bl: Vec4, c_br: Vec4,
}
#[derive(Asset, TypePath, AsBindGroup, Clone)]
struct FlowFieldMaterial { #[uniform(0)] uniforms: FlowFieldUniforms }
impl Material2d for FlowFieldMaterial {
    fn fragment_shader() -> ShaderRef { "shaders/flow_field.wgsl".into() }
}
#[derive(Resource)] struct FlowFieldMaterialHandle(Handle<FlowFieldMaterial>);
#[derive(Resource)] struct FlowFieldMeshHandle(Handle<Mesh>);

// ── Op Art material ────────────────────────────────────────────────────────────

#[derive(Clone, ShaderType)]
struct OpArtUniforms {
    ring_count: f32, ring_width: f32, warp: f32, freq: f32,
    twist: f32, _pad0: f32, _pad1: f32, _pad2: f32,
    fg_color: Vec4, bg_color: Vec4,
}
#[derive(Asset, TypePath, AsBindGroup, Clone)]
struct OpArtMaterial { #[uniform(0)] uniforms: OpArtUniforms }
impl Material2d for OpArtMaterial {
    fn fragment_shader() -> ShaderRef { "shaders/op_art.wgsl".into() }
}
#[derive(Resource)] struct OpArtMaterialHandle(Handle<OpArtMaterial>);
#[derive(Resource)] struct OpArtMeshHandle(Handle<Mesh>);

// ── Weave material ─────────────────────────────────────────────────────────────

#[derive(Clone, ShaderType)]
struct WeaveUniforms {
    thread_count: f32, thread_width: f32, shadow: f32, color_var: f32,
    _pad0: f32, _pad1: f32, _pad2: f32, _pad3: f32,
    warp_color: Vec4, weft_color: Vec4, bg_color: Vec4,
}
#[derive(Asset, TypePath, AsBindGroup, Clone)]
struct WeaveMaterial { #[uniform(0)] uniforms: WeaveUniforms }
impl Material2d for WeaveMaterial {
    fn fragment_shader() -> ShaderRef { "shaders/weave.wgsl".into() }
}
#[derive(Resource)] struct WeaveMaterialHandle(Handle<WeaveMaterial>);
#[derive(Resource)] struct WeaveMeshHandle(Handle<Mesh>);

// ── String Art material ────────────────────────────────────────────────────────

#[derive(Clone, ShaderType)]
struct StringArtUniforms {
    n_points: f32, k_offset: f32, line_width: f32, n_circles: f32,
    center_a: Vec4, center_b: Vec4, fg_color: Vec4, bg_color: Vec4,
}
#[derive(Asset, TypePath, AsBindGroup, Clone)]
struct StringArtMaterial { #[uniform(0)] uniforms: StringArtUniforms }
impl Material2d for StringArtMaterial {
    fn fragment_shader() -> ShaderRef { "shaders/string_art.wgsl".into() }
}
#[derive(Resource)] struct StringArtMaterialHandle(Handle<StringArtMaterial>);
#[derive(Resource)] struct StringArtMeshHandle(Handle<Mesh>);

// ── Reaction-Diffusion material ────────────────────────────────────────────────

#[derive(Clone, ShaderType)]
struct RdUniforms {
    scale: f32, sharpness: f32, warp: f32, balance: f32,
    _pad0: f32, _pad1: f32, _pad2: f32, _pad3: f32,
    fg_color: Vec4, bg_color: Vec4,
    c_tl: Vec4, c_tr: Vec4, c_bl: Vec4, c_br: Vec4,
}
#[derive(Asset, TypePath, AsBindGroup, Clone)]
struct RdMaterial { #[uniform(0)] uniforms: RdUniforms }
impl Material2d for RdMaterial {
    fn fragment_shader() -> ShaderRef { "shaders/reaction_diffusion.wgsl".into() }
}
#[derive(Resource)] struct RdMaterialHandle(Handle<RdMaterial>);
#[derive(Resource)] struct RdMeshHandle(Handle<Mesh>);

// ── Maze material ──────────────────────────────────────────────────────────────

#[derive(Clone, ShaderType)]
struct MazeUniforms {
    grid_w: u32, grid_h: u32, line_width: f32, _pad: f32,
    wall_color: Vec4, floor_color: Vec4,
    walls: [UVec4; 32],
}
#[derive(Asset, TypePath, AsBindGroup, Clone)]
struct MazeMaterial { #[uniform(0)] uniforms: MazeUniforms }
impl Material2d for MazeMaterial {
    fn fragment_shader() -> ShaderRef { "shaders/maze.wgsl".into() }
}
#[derive(Resource)] struct MazeMaterialHandle(Handle<MazeMaterial>);
#[derive(Resource)] struct MazeMeshHandle(Handle<Mesh>);

// ── Circuit Board material ─────────────────────────────────────────────────────

#[derive(Clone, ShaderType)]
struct CircuitUniforms {
    cell_size: f32, trace_width: f32, via_radius: f32, density: f32,
    color_var: f32, _pad0: f32, _pad1: f32, _pad2: f32,
    trace_color: Vec4, via_color: Vec4, bg_color: Vec4,
}
#[derive(Asset, TypePath, AsBindGroup, Clone)]
struct CircuitMaterial { #[uniform(0)] uniforms: CircuitUniforms }
impl Material2d for CircuitMaterial {
    fn fragment_shader() -> ShaderRef { "shaders/circuit.wgsl".into() }
}
#[derive(Resource)] struct CircuitMaterialHandle(Handle<CircuitMaterial>);
#[derive(Resource)] struct CircuitMeshHandle(Handle<Mesh>);

// ── Bubble Pack material ───────────────────────────────────────────────────────

#[derive(Clone, ShaderType)]
struct BubbleUniforms {
    density: f32, min_r: f32, max_r: f32, border: f32,
    jitter: f32, color_var: f32, _pad0: f32, _pad1: f32,
    c_tl: Vec4, c_tr: Vec4, c_bl: Vec4, c_br: Vec4,
}
#[derive(Asset, TypePath, AsBindGroup, Clone)]
struct BubbleMaterial { #[uniform(0)] uniforms: BubbleUniforms }
impl Material2d for BubbleMaterial {
    fn fragment_shader() -> ShaderRef { "shaders/bubble.wgsl".into() }
}
#[derive(Resource)] struct BubbleMaterialHandle(Handle<BubbleMaterial>);
#[derive(Resource)] struct BubbleMeshHandle(Handle<Mesh>);
...
impl Material2d for BlurMaterial {
    fn fragment_shader() -> ShaderRef { "shaders/blur.wgsl".into() }
}

#[derive(Resource)] struct BlurMaterialHandle(Handle<BlurMaterial>);
#[derive(Resource)] struct BlurMeshHandle(Handle<Mesh>);
/// The intermediate render target Camera A writes to.
#[derive(Resource)] struct SceneRenderTarget(Handle<Image>);
/// Entity ID of Camera A (scene → render target), needed for resize.
#[derive(Resource)] struct SceneCameraEntity(Entity);
...
fn setup(
    ...
    let blur_mat  = blur_materials.add(BlurMaterial {
        uniforms: BlurUniforms { radius: params.blur_radius, inv_w: 1.0/bw, inv_h: 1.0/bh, _pad: 0.0 },
        source:   rt,
    });
    commands.insert_resource(BlurMaterialHandle(blur_mat.clone()));
    commands.insert_resource(BlurMeshHandle(blur_mesh.clone()));
    commands.spawn((Mesh2d(blur_mesh), MeshMaterial2d(blur_mat), RenderLayers::layer(1)));
}
...
fn ui(
    ...
    egui::Window::new("Parameters")
        ...
        .show(ctx, |ui| {
            ...
            match params.tab {
                // ── Low Poly ──────────────────────────────────────────────────
                Tab::LowPoly => {
                    egui::Grid::new("lp").num_columns(2).spacing([8.0, 4.0]).show(ui, |ui| {
                        ...
                        fp_slider(ui, "Color variation", &mut params.lp_color_var, 0.0..=0.4, &mut dirty);
                ...
                // ── Voronoi ───────────────────────────────────────────────────
                Tab::Voronoi => {
                    egui::Grid::new("vor").num_columns(2).spacing([8.0, 4.0]).show(ui, |ui| {
                        ...
                        fp_slider(ui, "Dome height",     &mut params.vor_elevation, 0.0..=0.3, &mut dirty);
                        fp_slider(ui, "Color variation", &mut params.vor_color_var, 0.0..=0.2, &mut dirty);
                ...
                // ── Halftone ──────────────────────────────────────────────────
                Tab::Halftone => {
                    egui::Grid::new("ht").num_columns(2).spacing([8.0, 4.0]).show(ui, |ui| {
                        fp_slider(ui, "Tile frequency", &mut params.ht_tile_freq,   1.0..=12.0,  &mut ht_dirty);
                        fp_slider(ui, "Dot density",    &mut params.ht_dot_density, 10.0..=120.0,&mut ht_dirty);
                        fp_slider(ui, "Exponent",       &mut params.ht_exponent,    0.5..=5.0,   &mut ht_dirty);
                        fp_slider(ui, "Ambient",        &mut params.ht_ambient,     0.0..=0.5,   &mut ht_dirty);
                        fp_slider(ui, "Diffuse",        &mut params.ht_diffuse,     0.0..=1.0,   &mut ht_dirty);
                        fp_slider(ui, "Specular",       &mut params.ht_specular,    0.0..=1.0,   &mut ht_dirty);
                        fp_slider(ui, "Shininess",      &mut params.ht_shininess,   1.0..=128.0, &mut ht_dirty);
                        fp_slider_suffix(ui, "Light elevation", &mut params.ht_light_elev, 0.0..=90.0,  "°", &mut ht_dirty);
                        fp_slider_suffix(ui, "Light azimuth",   &mut params.ht_light_az,   0.0..=360.0, "°", &mut ht_dirty);
                        fp_slider(ui, "Color variation", &mut params.ht_color_var, 0.0..=0.2, &mut ht_dirty);
                ...
                // ── Topo ──────────────────────────────────────────────────────
                Tab::Topo => {
                    egui::Grid::new("tp").num_columns(2).spacing([8.0, 4.0]).show(ui, |ui| {
                        fp_slider(ui, "Noise scale",    &mut params.tp_noise_scale, 0.5..=10.0,  &mut tp_dirty);
                        fp_slider(ui, "Octaves",        &mut params.tp_octaves,     1.0..=8.0,   &mut tp_dirty);
                        fp_slider(ui, "Line count",     &mut params.tp_line_count,  4.0..=60.0,  &mut tp_dirty);
                        fp_slider(ui, "Line width",     &mut params.tp_line_width,  0.005..=0.2, &mut tp_dirty);
                        fp_slider(ui, "Ambient",        &mut params.tp_ambient,     0.0..=0.5,   &mut tp_dirty);
                        fp_slider(ui, "Diffuse",        &mut params.tp_diffuse,     0.0..=1.0,   &mut tp_dirty);
                        fp_slider(ui, "Specular",       &mut params.tp_specular,    0.0..=1.0,   &mut tp_dirty);
                        fp_slider(ui, "Shininess",      &mut params.tp_shininess,   1.0..=128.0, &mut tp_dirty);
                        fp_slider_suffix(ui, "Light elevation", &mut params.tp_light_elev, 0.0..=90.0,  "°", &mut tp_dirty);
                        fp_slider_suffix(ui, "Light azimuth",   &mut params.tp_light_az,   0.0..=360.0, "°", &mut tp_dirty);
                ...
                // ── Mosaic ────────────────────────────────────────────────────
                Tab::Mosaic => {
                    egui::Grid::new("ms").num_columns(2).spacing([8.0, 4.0]).show(ui, |ui| {
                        fp_slider(ui, "Tile count",      &mut params.ms_tile_count,  4.0..=80.0,  &mut ms_dirty);
                        fp_slider(ui, "Grout width",     &mut params.ms_grout_width, 0.01..=0.20, &mut ms_dirty);
                        fp_slider(ui, "Bevel width",     &mut params.ms_bevel_width, 0.0..=0.5,   &mut ms_dirty);
                        fp_slider(ui, "Noise scale",     &mut params.ms_noise_scale, 0.5..=8.0,   &mut ms_dirty);
                        fp_slider(ui, "Color variation", &mut params.ms_color_var,   0.0..=0.3,   &mut ms_dirty);
                        fp_slider(ui, "Ambient",         &mut params.ms_ambient,     0.0..=0.5,   &mut ms_dirty);
                        fp_slider(ui, "Diffuse",         &mut params.ms_diffuse,     0.0..=1.0,   &mut ms_dirty);
                        fp_slider(ui, "Specular",        &mut params.ms_specular,    0.0..=1.0,   &mut ms_dirty);
                        fp_slider(ui, "Shininess",       &mut params.ms_shininess,   1.0..=128.0, &mut ms_dirty);
                        fp_slider_suffix(ui, "Light elevation", &mut params.ms_light_elev, 0.0..=90.0,  "°", &mut ms_dirty);
                        fp_slider_suffix(ui, "Light azimuth",   &mut params.ms_light_az,   0.0..=360.0, "°", &mut ms_dirty);
                ...
                // ── Engrave ───────────────────────────────────────────────────
                Tab::Engrave => {
                    egui::Grid::new("eg").num_columns(2).spacing([8.0, 4.0]).show(ui, |ui| {
                        fp_slider(ui, "Noise scale",   &mut params.eg_noise_scale,  0.5..=8.0,    &mut eg_dirty);
                        fp_slider(ui, "Octaves",       &mut params.eg_octaves,      1.0..=8.0,    &mut eg_dirty);
                        fp_slider(ui, "Line count",    &mut params.eg_line_count,   4.0..=60.0,   &mut eg_dirty);
                        fp_slider(ui, "Dot density",   &mut params.eg_dot_density,  20.0..=300.0, &mut eg_dirty);
                        fp_slider(ui, "Min dot radius",&mut params.eg_dot_min_r,    0.0..=0.4,    &mut eg_dirty);
                        fp_slider(ui, "Max dot radius",&mut params.eg_dot_max_r,    0.1..=0.5,    &mut eg_dirty);
                ...
                // ── WFC ───────────────────────────────────────────────────────
                Tab::Wfc => {
                    egui::Grid::new("wfc").num_columns(2).spacing([8.0, 4.0]).show(ui, |ui| {
                        lp_slider(ui, "Grid width",       &mut params.wfc_grid_w,    4..=32,      &mut wfc_grid_dirty);
                        lp_slider(ui, "Grid height",      &mut params.wfc_grid_h,    4..=24,      &mut wfc_grid_dirty);
                        fp_slider(ui, "Line width",       &mut params.wfc_line_width, 0.02..=0.45, &mut wfc_vis_dirty);
                        fp_slider(ui, "Color variation",  &mut params.wfc_color_var,  0.0..=0.3,   &mut wfc_vis_dirty);

                        ui.label("Tile set");
                        let changed =
                            ui.radio_value(&mut params.wfc_tile_set, 0, "Pipes").changed()
                            | ui.radio_value(&mut params.wfc_tile_set, 1, "+Cross").changed()
                            | ui.radio_value(&mut params.wfc_tile_set, 2, "+T-junc").changed();
                        if changed { wfc_grid_dirty = true; }
                        ui.end_row();

                        color_row(ui, "Pipe color", &mut params.wfc_fg_color, &mut wfc_vis_dirty);
                        color_row(ui, "Background", &mut params.wfc_bg_color, &mut wfc_vis_dirty);
                    });
                }

                // ── Crackle ───────────────────────────────────────────────────
                Tab::Crackle => {
                    egui::Grid::new("ck").num_columns(2).spacing([8.0, 4.0]).show(ui, |ui| {
                        fp_slider(ui, "Cell count",        &mut params.ck_cell_count,   4.0..=40.0,  &mut ck_dirty);
                        fp_slider(ui, "Crack width",       &mut params.ck_crack_width,  0.01..=0.3,  &mut ck_dirty);
                        fp_slider(ui, "Jitter",            &mut params.ck_jitter,       0.0..=1.0,   &mut ck_dirty);
                        fp_slider(ui, "Domain warp",       &mut params.ck_warp,         0.0..=1.5,   &mut ck_dirty);
                        fp_slider(ui, "Color variation",   &mut params.ck_color_var,    0.0..=0.3,   &mut ck_dirty);
                        fp_slider(ui, "Inner vein width",  &mut params.ck_inner_width,  0.0..=0.5,   &mut ck_dirty);
                        fp_slider(ui, "Inner vein bright", &mut params.ck_inner_bright, 0.0..=1.0,   &mut ck_dirty);
                        fp_slider(ui, "Cell gradient",     &mut params.ck_cell_depth,   0.0..=1.0,   &mut ck_dirty);
                ...
                // ── Flow Field ────────────────────────────────────────────────
                Tab::FlowField => {
                    egui::Grid::new("ff").num_columns(2).spacing([8.0, 4.0]).show(ui, |ui| {
                        fp_slider(ui, "Frequency",       &mut params.ff_freq,       0.5..=10.0,  &mut ff_dirty);
                        fp_slider(ui, "Line count",      &mut params.ff_n_lines,    2.0..=40.0,  &mut ff_dirty);
                        fp_slider(ui, "Line width",      &mut params.ff_line_width, 0.01..=0.5,  &mut ff_dirty);
                        fp_slider(ui, "Domain warp",     &mut params.ff_warp,       0.0..=2.0,   &mut ff_dirty);
                        fp_slider(ui, "Octaves",         &mut params.ff_octaves,    1.0..=8.0,   &mut ff_dirty);
                        fp_slider(ui, "Color variation", &mut params.ff_color_var,  0.0..=0.3,   &mut ff_dirty);
                ...
                // ── Op Art ────────────────────────────────────────────────────
                Tab::OpArt => {
                    egui::Grid::new("oa").num_columns(2).spacing([8.0, 4.0]).show(ui, |ui| {
                        fp_slider(ui, "Ring count",  &mut params.oa_ring_count, 2.0..=24.0,  &mut oa_dirty);
                        fp_slider(ui, "Ring width",  &mut params.oa_ring_width, 0.02..=1.0,  &mut oa_dirty);
                        fp_slider(ui, "Warp",        &mut params.oa_warp,       0.0..=0.6,   &mut oa_dirty);
                        fp_slider(ui, "Warp freq",   &mut params.oa_freq,       0.5..=8.0,   &mut oa_dirty);
                ...
                // ── Weave ─────────────────────────────────────────────────────
                Tab::Weave => {
                    egui::Grid::new("wv").num_columns(2).spacing([8.0, 4.0]).show(ui, |ui| {
                        fp_slider(ui, "Thread count", &mut params.wv_thread_count, 4.0..=50.0, &mut wv_dirty);
                        fp_slider(ui, "Thread width", &mut params.wv_thread_width, 0.1..=0.9,  &mut wv_dirty);
                        fp_slider(ui, "Shadow",       &mut params.wv_shadow,       0.0..=0.8,  &mut wv_dirty);
                        fp_slider(ui, "Color var",    &mut params.wv_color_var,    0.0..=0.3,  &mut wv_dirty);
                        color_row(ui, "Warp (vertical)",   &mut params.wv_warp_color, &mut wv_dirty);
                        color_row(ui, "Weft (horizontal)", &mut params.wv_weft_color, &mut wv_dirty);
                ...
                // ── String Art ────────────────────────────────────────────────
                Tab::StringArt => {
                    egui::Grid::new("sa").num_columns(2).spacing([8.0, 4.0]).show(ui, |ui| {
                        fp_slider(ui, "Points",      &mut params.sa_n_points,   20.0..=256.0, &mut sa_dirty);
                        fp_slider(ui, "Offset k",    &mut params.sa_k_offset,   2.0..=200.0,  &mut sa_dirty);
                        fp_slider(ui, "Line width",  &mut params.sa_line_width, 0.001..=0.012,&mut sa_dirty);
                        ui.label("Circles");
                        let changed = ui.radio_value(&mut params.sa_n_circles, 1.0, "One").changed()
                                    | ui.radio_value(&mut params.sa_n_circles, 2.0, "Two").changed();
                        if changed { sa_dirty = true; }
                        ui.end_row();
                        fp_slider(ui, "Circle A x",  &mut params.sa_cx_a, 0.1..=0.9, &mut sa_dirty);
                        fp_slider(ui, "Circle A y",  &mut params.sa_cy_a, 0.1..=0.9, &mut sa_dirty);
                        fp_slider(ui, "Circle A r",  &mut params.sa_r_a,  0.05..=0.48,&mut sa_dirty);
                        fp_slider(ui, "Circle B x",  &mut params.sa_cx_b, 0.1..=0.9, &mut sa_dirty);
                        fp_slider(ui, "Circle B y",  &mut params.sa_cy_b, 0.1..=0.9, &mut sa_dirty);
                        fp_slider(ui, "Circle B r",  &mut params.sa_r_b,  0.05..=0.48,&mut sa_dirty);
                ...
                // ── Reaction-Diffusion ────────────────────────────────────────
                Tab::RD => {
                    egui::Grid::new("rd").num_columns(2).spacing([8.0, 4.0]).show(ui, |ui| {
                        fp_slider(ui, "Scale",     &mut params.rd_scale,     1.0..=10.0,  &mut rd_dirty);
                        fp_slider(ui, "Sharpness", &mut params.rd_sharpness, 1.0..=20.0,  &mut rd_dirty);
                        fp_slider(ui, "Warp",      &mut params.rd_warp,      0.0..=1.5,   &mut rd_dirty);
                        fp_slider(ui, "Balance",   &mut params.rd_balance,   -0.5..=0.5,  &mut rd_dirty);
                ...
                // ── Maze ──────────────────────────────────────────────────────
                Tab::Maze => {
                    egui::Grid::new("mz").num_columns(2).spacing([8.0, 4.0]).show(ui, |ui| {
                        lp_slider(ui, "Grid width",  &mut params.mz_grid_w,    4..=32,      &mut mz_grid_dirty);
                        lp_slider(ui, "Grid height", &mut params.mz_grid_h,    4..=24,      &mut mz_grid_dirty);
                        fp_slider(ui, "Wall width",  &mut params.mz_line_width, 0.01..=0.45,&mut mz_vis_dirty);
                        color_row(ui, "Wall color",  &mut params.mz_wall_color,  &mut mz_vis_dirty);
                        color_row(ui, "Floor color", &mut params.mz_floor_color, &mut mz_vis_dirty);
                    });
                }

                // ── Circuit Board ─────────────────────────────────────────────
                Tab::Circuit => {
                    egui::Grid::new("cb").num_columns(2).spacing([8.0, 4.0]).show(ui, |ui| {
                        fp_slider(ui, "Grid density",  &mut params.cb_cell_size,   4.0..=30.0,  &mut cb_dirty);
                        fp_slider(ui, "Trace width",   &mut params.cb_trace_width, 0.02..=0.4,  &mut cb_dirty);
                        fp_slider(ui, "Via radius",    &mut params.cb_via_radius,  0.05..=0.3,  &mut cb_dirty);
                        fp_slider(ui, "Density",       &mut params.cb_density,     0.1..=1.0,   &mut cb_dirty);
                        fp_slider(ui, "Color var",     &mut params.cb_color_var,   0.0..=0.3,   &mut cb_dirty);
                ...
                // ── Bubble Pack ───────────────────────────────────────────────
                Tab::Bubble => {
                    egui::Grid::new("bp").num_columns(2).spacing([8.0, 4.0]).show(ui, |ui| {
                        fp_slider(ui, "Density",       &mut params.bp_density,   3.0..=20.0,  &mut bp_dirty);
                        fp_slider(ui, "Min radius",    &mut params.bp_min_r,     0.05..=0.4,  &mut bp_dirty);
                        fp_slider(ui, "Max radius",    &mut params.bp_max_r,     0.15..=0.48, &mut bp_dirty);
                        fp_slider(ui, "Border",        &mut params.bp_border,    0.0..=0.15,  &mut bp_dirty);
                        fp_slider(ui, "Jitter",        &mut params.bp_jitter,    0.0..=1.0,   &mut bp_dirty);
                        fp_slider(ui, "Color var",     &mut params.bp_color_var, 0.0..=0.3,   &mut bp_dirty);
                ...
                // ── Penrose ───────────────────────────────────────────────────
                Tab::Penrose => {
                    egui::Grid::new("pe").num_columns(2).spacing([8.0, 4.0]).show(ui, |ui| {
                        lp_slider(ui, "Iterations",    &mut params.pe_iterations, 2..=7,     &mut dirty);
                        fp_slider(ui, "Color var",     &mut params.pe_color_var,  0.0..=0.3,  &mut dirty);
                ...
                // ── Output ────────────────────────────────────────────────────
                Tab::Output => {
                    egui::Grid::new("out").num_columns(2).spacing([8.0, 4.0]).show(ui, |ui| {
                        ui.label("Width");
                        if ui.add(egui::Slider::new(&mut params.out_w, 320..=3840)).changed() {
                            dirty = true; resize = true;
                        }
                        ui.end_row();
                        ui.label("Height");
                        if ui.add(egui::Slider::new(&mut params.out_h, 240..=2160)).changed() {
                            dirty = true; resize = true;
                        }
                        ui.end_row();
                        ui.label("Blur radius");
                        if ui.add(egui::Slider::new(&mut params.blur_radius, 0.0..=20.0).suffix("px")).changed() {
                            blur_dirty = true;
                        }
                        ui.end_row();
                    });
                    ...
                    if matches!(params.style, Style::LowPoly | Style::Voronoi) {
                        ...
                        ui.add_space(2.0);
                        egui::Grid::new("out_col").num_columns(2).spacing([8.0, 4.0]).show(ui, |ui| {
                            match params.style {
                                Style::LowPoly => {
                                    color_row(ui, "Top-left",     &mut params.lp_c_tl, &mut dirty);
                                    color_row(ui, "Top-right",    &mut params.lp_c_tr, &mut dirty);
            ...
            ui.separator();
            ui.horizontal(|ui| {
                if matches!(params.style, Style::LowPoly | Style::Voronoi | Style::Wfc | Style::Maze | Style::Penrose)
    ...
    if params.tab != prev_tab {
        match params.tab {
            Tab::LowPoly   => { params.style = Style::LowPoly;    dirty         = true; }
            Tab::Voronoi   => { params.style = Style::Voronoi;    dirty         = true; }
            Tab::Halftone  => { params.style = Style::Halftone;   ht_dirty      = true; }
            Tab::Topo      => { params.style = Style::Topo;       tp_dirty      = true; }
            Tab::Mosaic    => { params.style = Style::Mosaic;     ms_dirty      = true; }
            Tab::Engrave   => { params.style = Style::Engrave;    eg_dirty      = true; }
            Tab::Wfc       => { params.style = Style::Wfc;        wfc_vis_dirty = true; }
            Tab::Crackle   => { params.style = Style::Crackle;    ck_dirty      = true; }
            Tab::FlowField => { params.style = Style::FlowField;  ff_dirty      = true; }
            Tab::OpArt     => { params.style = Style::OpArt;      oa_dirty      = true; }
            Tab::Weave     => { params.style = Style::Weave;      wv_dirty      = true; }
            Tab::StringArt => { params.style = Style::StringArt;  sa_dirty      = true; }
            Tab::RD        => { params.style = Style::RD;         rd_dirty      = true; }
            Tab::Maze      => { params.style = Style::Maze;       mz_vis_dirty  = true; }
            Tab::Circuit   => { params.style = Style::Circuit;    cb_dirty      = true; }
            Tab::Bubble    => { params.style = Style::Bubble;     bp_dirty      = true; }
            Tab::Penrose   => { params.style = Style::Penrose;    dirty         = true; }
            Tab::Output    => {}
        }
    }
    ...
    if resize {
        ...
        if let Some(mesh) = meshes.get_mut(&handles.ck_mesh.0) { *mesh = quad.clone(); }
        if let Some(mesh) = meshes.get_mut(&handles.ff_mesh.0) { *mesh = quad.clone(); }
        if let Some(mesh) = meshes.get_mut(&handles.oa_mesh.0) { *mesh = quad.clone(); }
        if let Some(mesh) = meshes.get_mut(&handles.wv_mesh.0) { *mesh = quad.clone(); }
        if let Some(mesh) = meshes.get_mut(&handles.sa_mesh.0) { *mesh = quad.clone(); }
        if let Some(mesh) = meshes.get_mut(&handles.rd_mesh.0) { *mesh = quad.clone(); }
        if let Some(mesh) = meshes.get_mut(&handles.mz_mesh.0) { *mesh = quad.clone(); }
        if let Some(mesh) = meshes.get_mut(&handles.cb_mesh.0) { *mesh = quad.clone(); }
        if let Some(mesh) = meshes.get_mut(&handles.bp_mesh.0) { *mesh = quad.clone(); }
    ...
    if dirty && matches!(params.style, Style::LowPoly | Style::Voronoi | Style::Penrose) {
    ...
    if ht_dirty {
        if let Some(mat) = mats.ht.get_mut(&handles.ht_mat.0) { mat.uniforms = params.to_ht_uniforms(); }
    }
    if tp_dirty {
        if let Some(mat) = mats.tp.get_mut(&handles.tp_mat.0) { mat.uniforms = params.to_topo_uniforms(); }
    }
    if ms_dirty {
        if let Some(mat) = mats.ms.get_mut(&handles.ms_mat.0) { mat.uniforms = params.to_mosaic_uniforms(); }
    }
    if eg_dirty {
        if let Some(mat) = mats.eg.get_mut(&handles.eg_mat.0) { mat.uniforms = params.to_engrave_uniforms(); }
    }
    if wfc_grid_dirty {
        let new_grid = run_wfc(params.wfc_grid_w, params.wfc_grid_h, wfc_tiles(params.wfc_tile_set), params.seed);
        params.wfc_grid = new_grid;
    }
    if wfc_grid_dirty || wfc_vis_dirty {
        if let Some(mat) = mats.wfc.get_mut(&handles.wfc_mat.0) { mat.uniforms = params.to_wfc_uniforms(); }
    }
    if ck_dirty {
        if let Some(mat) = mats.ck.get_mut(&handles.ck_mat.0) { mat.uniforms = params.to_crackle_uniforms(); }
    }
    if ff_dirty {
        if let Some(mat) = mats.ff.get_mut(&handles.ff_mat.0) { mat.uniforms = params.to_ff_uniforms(); }
    }
    if oa_dirty {
        if let Some(mat) = mats.oa.get_mut(&handles.oa_mat.0) { mat.uniforms = params.to_oa_uniforms(); }
    }
    if wv_dirty {
        if let Some(mat) = mats.wv.get_mut(&handles.wv_mat.0) { mat.uniforms = params.to_wv_uniforms(); }
    }
    if sa_dirty {
        if let Some(mat) = mats.sa.get_mut(&handles.sa_mat.0) { mat.uniforms = params.to_sa_uniforms(); }
    }
    if rd_dirty {
        if let Some(mat) = mats.rd.get_mut(&handles.rd_mat.0) { mat.uniforms = params.to_rd_uniforms(); }
    }
    if mz_grid_dirty {
        params.mz_grid = run_maze(params.mz_grid_w, params.mz_grid_h, params.seed);
    }
    if mz_grid_dirty || mz_vis_dirty {
        if let Some(mat) = mats.mz.get_mut(&handles.mz_mat.0) { mat.uniforms = params.to_mz_uniforms(); }
    }
    if cb_dirty {
        if let Some(mat) = mats.cb.get_mut(&handles.cb_mat.0) { mat.uniforms = params.to_cb_uniforms(); }
    }
    if bp_dirty {
        if let Some(mat) = mats.bp.get_mut(&handles.bp_mat.0) { mat.uniforms = params.to_bp_uniforms(); }
    }
    if blur_dirty {
        if let Some(mat) = post.blur_mat.get_mut(&post.blur_h.0) {
    ...
    for (mut vis, id) in sv.q.iter_mut() {
        *vis = if id.0 == style { Visibility::Visible } else { Visibility::Hidden };
    }
...
fn lp_slider(ui: &mut egui::Ui, label: &str, v: &mut usize, r: std::ops::RangeInclusive<usize>, d: &mut bool) {
    ui.label(label);
    *d |= ui.add(egui::Slider::new(v, r)).changed();
    ui.end_row();
}
fn fp_slider(ui: &mut egui::Ui, label: &str, v: &mut f32, r: std::ops::RangeInclusive<f32>, d: &mut bool) {
    ui.label(label);
    *d |= ui.add(egui::Slider::new(v, r)).changed();
    ui.end_row();
}
fn fp_slider_suffix(ui: &mut egui::Ui, label: &str, v: &mut f32, r: std::ops::RangeInclusive<f32>, suffix: &str, d: &mut bool) {
    ui.label(label);
...
fn dispatch_mesh(params: &Params) -> Mesh {
    match params.style {
        ...
        _               => Mesh::new(PrimitiveTopology::TriangleList, RenderAssetUsages::RENDER_WORLD),
    }
}
...
fn build_lowpoly(p: &Params) -> Mesh {
    ...
    for row in 0..=rows {
        for col in 0..=cols {
            ...
            let jx = if col > 0 && col < cols { rng.gen_range(-0.5..0.5) * jitter * cw } else { 0.0 };
            let jy = if row > 0 && row < rows { rng.gen_range(-0.5..0.5) * jitter * ch } else { 0.0 };
...
fn lp_tri(pos: &mut Vec<[f32; 3]>, col: &mut Vec<[f32; 4]>,
          a: [f32; 2], b: [f32; 2], c: [f32; 2],
          p: &Params, rng: &mut StdRng, w: f32, h: f32) {
    ...
    let clr = [(r+v).clamp(0.,1.), (g+v).clamp(0.,1.), (bv+v).clamp(0.,1.), 1.0];
    for pt in [a, b, c] { pos.push([pt[0], pt[1], 0.]); col.push(clr); }
}
...
fn build_voronoi(p: &Params) -> Mesh {
    ...
    for row in 0..nr {
        for col in 0..nc {
            let bx = col as f32 * cw - w * 0.5;
            let by = row as f32 * ch - h * 0.5;
            let jx = if col > 0 && col < nc-1 { rng.gen_range(-0.5..0.5) * jitter * cw } else { 0. };
            let jy = if row > 0 && row < nr-1 { rng.gen_range(-0.5..0.5) * jitter * ch } else { 0. };
    ...
    for row in 0..nr {
        for c in 0..nc {
            ...
            if inset.len() < 3 || poly_area(&inset).abs() < 4.0 { continue; }

            let [r, g, bv, _] = bilinear(sp[0], sp[1], w, h,
                p.vor_c_tl, p.vor_c_tr, p.vor_c_bl, p.vor_c_br);
            let v    = rng.gen_range(-p.vor_color_var..p.vor_color_var);
            let base = [(r+v).clamp(0.,1.), (g+v).clamp(0.,1.), (bv+v).clamp(0.,1.), 1.0_f32];
            let e    = p.vor_elevation;
            let hi   = [(base[0]+e).clamp(0.,1.), (base[1]+e).clamp(0.,1.),
                        (base[2]+e).clamp(0.,1.), 1.0];

            let n = inset.len();
            for i in 0..n {
                let a = inset[i];
                let b = inset[(i + 1) % n];
                pos.push([centroid[0], centroid[1], 0.]); col.push(hi);
                pos.push([a[0], a[1], 0.]);               col.push(base);
                pos.push([b[0], b[1], 0.]);               col.push(base);
            }
        }
    }
...
fn voronoi_cell(seeds: &[[f32; 2]], row: usize, col: usize,
                nr: usize, nc: usize, w: f32, h: f32) -> Vec<[f32; 2]> {
    let seed = seeds[row * nc + col];
    let pad  = 50.0;
    let mut poly = vec![
        [-w*0.5-pad, -h*0.5-pad], [ w*0.5+pad, -h*0.5-pad],
        [ w*0.5+pad,  h*0.5+pad], [-w*0.5-pad,  h*0.5+pad],
    ];
    for dr in -1i32..=1 {
        for dc in -1i32..=1 {
            ...
            if nr2 < 0 || nr2 >= nr as i32 || nc2 < 0 || nc2 >= nc as i32 { continue; }
...
fn clip_halfplane(poly: Vec<[f32; 2]>, seed: [f32; 2], nbr: [f32; 2]) -> Vec<[f32; 2]> {
    ...
    let isect = |a: [f32; 2], b: [f32; 2]| -> [f32; 2] {
        let da = dot(a); let db = dot(b); let t = da / (da - db);
    ...
    for i in 0..n {
        ...
        let ai = dot(a) >= 0.0; let bi = dot(b) >= 0.0;
...
fn poly_area(poly: &[[f32; 2]]) -> f32 {
    let n = poly.len();
    let mut a = 0.0f32;
    for i in 0..n { let j=(i+1)%n; a += poly[i][0]*poly[j][1] - poly[j][0]*poly[i][1]; }
    a * 0.5
}
...
fn run_wfc(w: usize, h: usize, tiles: &[u8], seed: u64) -> Vec<u8> {
    ...
    (0..w*h).map(|i| if (i/w + i%w) % 2 == 0 { h_t } else { v_t }).collect()
}
...
fn try_wfc(w: usize, h: usize, tiles: &[u8], seed: u64) -> Option<Vec<u8>> {
    ...
    for _ in 0..n {
        let cell = (0..n).filter(|&i| wave[i].len() > 1).min_by_key(|&i| wave[i].len());
        let Some(cell) = cell else { break };
        if wave[cell].is_empty() { return None; }
    ...
    Some(wave.into_iter().map(|opts| opts.into_iter().next().unwrap_or(tiles[0])).collect())
}

// ── Shared ────────────────────────────────────────────────────────────────────

fn bilinear(x: f32, y: f32, w: f32, h: f32,
            c_tl: [f32; 3], c_tr: [f32; 3], c_bl: [f32; 3], c_br: [f32; 3]) -> [f32; 4] {
...
fn run_maze(w: usize, h: usize, seed: u64) -> Vec<u8> {
    ...
    while let Some(&(cx, cy)) = stack.last() {
        let neighbors: Vec<_> = dirs.iter().filter_map(|&(dx, dy, wf, wt)| {
            let nx = cx as i32 + dx;
            let ny = cy as i32 + dy;
            if nx >= 0 && ny >= 0 && nx < w as i32 && ny < h as i32 {
                let ni = ny as usize * w + nx as usize;
                if !visited[ni] { return Some((nx as usize, ny as usize, wf, wt)); }
            }
            None
        }).collect();
...
#[derive(Clone, Copy)]
enum PTriKind { Red, Blue }

#[derive(Clone)]
struct PTri { kind: PTriKind, v: [Vec2; 3] }
    ...
    for t in tris {
        ...
        match t.kind {
            PTriKind::Red => {
                ...
                let p = a + (b - a) / phi;
                out.push(PTri { kind: PTriKind::Red,  v: [c, p, b] });
                out.push(PTri { kind: PTriKind::Blue, v: [p, c, a] });
            }
            PTriKind::Blue => {
                // q on BA, r on BC
                let q = b + (a - b) / phi;
                let r = b + (c - b) / phi;
                out.push(PTri { kind: PTriKind::Blue, v: [r, c, a] });
                out.push(PTri { kind: PTriKind::Blue, v: [q, r, b] });
                out.push(PTri { kind: PTriKind::Red,  v: [r, q, a] });
            }
        }
    }
    out
...
fn build_penrose(p: &Params) -> Mesh {
    ...
    for t in &tris {
        ...
        if xs.iter().all(|&x| x < -half_w) || xs.iter().all(|&x| x > half_w) { continue; }
        if ys.iter().all(|&y| y < -half_h) || ys.iter().all(|&y| y > half_h) { continue; }
        ...
        let c = [(base[0]+v).clamp(0.,1.), (base[1]+v).clamp(0.,1.), (base[2]+v).clamp(0.,1.), 1.0_f32];
        for vi in &t.v {
